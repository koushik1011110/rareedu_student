...
const Visa = () => {
  ...
  const [residencyData, setResidencyData] = useState<ResidencyData | null>(
    null,
  );
  ...
  useEffect(() => {
    const fetchVisaData = async () => {
      ...
      try {
        ...
        const [visaResult, deadlinesResult, documentsResult] =
        ...
        if (visaResult.status === "fulfilled") {
          const { data: visaInfo, error: visaError } = visaResult.value;
          if (visaError && visaError.code !== "PGRST116") {
            console.error("Error fetching visa data:", visaError);
          } else if (visaInfo) {
            setVisaData(visaInfo);

            // Create residency data from the visa data
            const residencyInfo = {
              registration_status: visaInfo.residency_registration
                ? "Registered"
                : "Pending Registration",
              registration_deadline: "2025-09-15", // Default deadline
              current_address: "123 University Ave, College Town, ST 12345", // Default address
        ...
        if (deadlinesResult.status === "fulfilled") {
          const { data: deadlinesData, error: deadlinesError } =
  ...
  const daysUntilRegistration = residencyData?.registration_deadline
    ? differenceInDays(
        new Date(residencyData.registration_deadline),
        new Date(),
      )
    : 0;
  ...
  const timelineEvents = [
    {
      ...
      date: visaData?.application_date
        ? new Date(visaData.application_date)
        : null,
      status: visaData?.application_submitted
        ? "completed"
        : visaData?.application_date
          ? "in-progress"
          : "pending",
    ...
    },
    {
      ...
      status: visaData?.visa_interview
        ? "completed"
        : visaData?.interview_date
          ? "in-progress"
          : visaData?.application_submitted
            ? "upcoming"
            : "pending",
    ...
    },
    {
      ...
      status: visaData?.visa_approved
        ? "completed"
        : visaData?.approval_date
          ? "in-progress"
          : visaData?.visa_interview
            ? "upcoming"
            : "pending",
    ...
    },
    {
      ...
      date: residencyData?.registration_deadline
        ? new Date(residencyData.registration_deadline)
        : null,
      status: visaData?.residency_registration
        ? "completed"
        : residencyData?.registration_status === "Registered"
          ? "completed"
          : visaData?.visa_approved
            ? "upcoming"
            : "pending",
    ...
    <div className="pb-20 md:pb-6">
      ...
      >
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
          <div>
            ...
            <p className="text-gray-600">
              Track your visa status and residency requirements
            </p>
          </div>
        </div>
              ...
              <div className="flex justify-between items-start">
                ...
                <span
                  className={`status-badge ${
                    visaData?.visa_status === "Approved"
                      ? "status-approved"
                      : visaData?.visa_status === "Pending"
                        ? "status-pending"
                        : "status-rejected"
                  }`}
                >
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <p className="font-medium">
                        {visaData.issue_date
                          ? format(new Date(visaData.issue_date), "MMM d, yyyy")
                          : "N/A"}
                      </p>
                    </div>
                    <div>
                      ...
                      <p className="font-medium">
                        {visaData.expiration_date
                          ? format(
                              new Date(visaData.expiration_date),
                              "MMM d, yyyy",
                            )
                          : "N/A"}
                      </p>
                        ...
                        <div className="mt-4 p-3 bg-warning-50 text-warning-800 rounded-lg flex items-start">
                          ...
                          <div>
                            ...
                            <p className="text-sm mt-1">
                              Your visa will expire in {daysUntilExpiration}{" "}
                              days. Please start the renewal process at least 60
          ...
          <div className="card relative overflow-hidden">
            ...
            <div className="relative">
              <div className="flex justify-between items-start">
                ...
                <span
                  className={`status-badge ${
                    residencyData?.registration_status === "Registered"
                      ? "status-approved"
                      : residencyData?.registration_status ===
                          "Pending Registration"
                        ? "status-pending"
                        : "status-expired"
                  }`}
              ...
              {residencyData ? (
                <>
                  <div className="mt-4 space-y-4">
                    <div>
                      ...
                      <p className="font-medium">
                        {residencyData.registration_deadline
                          ? format(
                              new Date(residencyData.registration_deadline),
                              "MMM d, yyyy",
                            )
                    ...
                    <div>
                      ...
                      <p className="font-medium">
                        {residencyData.current_address}
                      </p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-500">Local ID Number</p>
                      <p className="font-medium">
                        {residencyData.local_id_number}
                      </p>
                    </div>
                  </div>

                  <div className="mt-6 pt-4 border-t border-gray-100">
                    {daysUntilRegistration <= 30 &&
                    daysUntilRegistration > 0 ? (
                      <div className="p-3 bg-warning-50 text-warning-800 rounded-lg flex items-start">
                        ...
                        <div>
                          ...
                          <p className="text-sm mt-1">
                            You must complete local residency registration
                            within {daysUntilRegistration} days.
                          </p>
                          <button className="mt-2 text-sm font-medium text-primary-600 hover:text-primary-700 inline-flex items-center">
                ...
                <div className="mt-4 p-4 bg-gray-50 rounded-lg text-center">
                  <p className="text-gray-600">
                    No residency information available
                  </p>
                </div>
              )}
            </div>
                    ...
                    {timelineEvents.map((event, index) => (
                      ...
                      >
                        ...
                        {index < timelineEvents.length - 1 && (
                          <div
                            className={`absolute h-full w-0.5 left-6 top-6 ${event.status === "completed" ? "bg-success-500" : "bg-gray-200"}`}
                          ></div>
                        )}
                        <div
                          className={`absolute h-4 w-4 rounded-full border-2 border-white left-4 mt-1.5 z-10 ${
                ...
                <div className="space-y-4">
                  <div className="p-4 border border-gray-200 rounded-lg">
                    ...
                    <p className="text-sm text-gray-500 mt-1">
                      For visa and residency inquiries
                    </p>
                  ...
                  <div className="p-4 border border-gray-200 rounded-lg">
                    ...
                    <div className="mt-2 pt-2 border-t border-gray-100">
                      <p className="text-sm">
                        University Security: +1 (555) 987-6543
                      </p>
        ...
        {activeTab === "deadlines" && (
          <div className="card">
            <h2 className="text-lg font-semibold text-gray-900 mb-4">
              Upcoming Deadlines & Requirements
            </h2>
            ...
            <div className="mt-8">
              ...
              <div className="bg-skyblue-50 p-4 rounded-lg mb-4">
                <h4 className="font-medium text-skyblue-900">
                  Study Requirements
                </h4>
                <ul className="mt-2 text-sm text-skyblue-800 space-y-2">
                  <li className="flex items-start">
                    ...
                    Maintain full-time enrollment (minimum 12 credit hours per
                    semester)
                  </li>
                  <li className="flex items-start">
                    <span className="h-5 w-5 rounded-full bg-skyblue-200 flex items-center justify-center mr-2 flex-shrink-0 mt-0.5">
                  ...
                  <li className="flex items-start">
                    ...
                    Limited work permission (max 20 hours/week on-campus during
                    semester)
                  </li>
                </ul>
              </div>
              ...
              <div>
                <div className="card">
                  ...
                  <div className="p-4 bg-primary-50 rounded-lg mb-6">
                    ...
                    <ul className="text-sm text-primary-700 space-y-2">
                      <li className="flex items-start">
                        ...
                        />
                        Always carry your passport and I-20/DS-2019 when
                      ...
                      <li className="flex items-start">
                        <CheckCircle
                          ...
                          className="mr-2 mt-0.5 flex-shrink-0"
                        />
                        Maintain full-time enrollment to keep your visa status
                        valid
                      </li>
                      <li className="flex items-start">
                        <CheckCircle
                          ...
                          className="mr-2 mt-0.5 flex-shrink-0"
                        />
                        Start visa renewal process at least 3 months before
                        expiration
                      </li>
                    </ul>
                  </div>
                  ...
                  <div className="p-4 bg-skyblue-50 rounded-lg">
                    <h4 className="font-medium text-skyblue-800 mb-2">
                      Residency Registration Process
                    </h4>
